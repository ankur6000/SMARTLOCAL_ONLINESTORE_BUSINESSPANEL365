...
const planQuickSelect = document.getElementById('plan-quick-select');
const registrationPlanSelect = document.getElementById('reg-plan-select');
const planQuickMeta = document.getElementById('plan-quick-meta');
const registrationPlanMeta = document.getElementById('reg-plan-meta');
...
function getPlanSummary(planType) {
  const tier = String(planType || '').toLowerCase();
  if (tier === 'silver') return 'Silver plan at Rs. 999 per month for essential business tools.';
  if (tier === 'gold') return 'Gold plan at Rs. 2,499 per month for automation, courier, and ROBERT-ready workflow.';
  if (tier === 'platinum') return 'Platinum plan at Rs. 4,999 per month for premium analytics and advanced control.';
  return 'Choose Silver, Gold, or Platinum to continue registration.';
}

function syncPlanSelectors(planType) {
  const tier = String(planType || '').toLowerCase();
  if (planQuickSelect && planQuickSelect.value !== tier) planQuickSelect.value = tier;
  if (registrationPlanSelect && registrationPlanSelect.value !== tier) registrationPlanSelect.value = tier;
  if (planQuickMeta) {
    planQuickMeta.innerHTML = `<strong>${tier ? getPlanSegment(tier) : 'Select a segment'}</strong><span>${getPlanSummary(tier)}</span>`;
  }
  if (registrationPlanMeta) {
    registrationPlanMeta.innerHTML = `<strong>${tier ? getPlanSegment(tier) : 'Select a segment'}</strong><span>${getPlanSummary(tier)}</span>`;
  }
}
...
function selectPlan(planType, showToastNotification = true) {
  ...
  syncPlanSelectors(normalizedPlan);
...
if (planQuickSelect) {
  const handleQuickPlanChange = function() {
    const nextPlan = String(this.value || '').toLowerCase();
    if (!nextPlan) {
      goBackToPlans();
      return;
    }
    selectPlan(nextPlan, false);
  };
  planQuickSelect.addEventListener('change', handleQuickPlanChange);
  planQuickSelect.addEventListener('input', handleQuickPlanChange);
}

if (registrationPlanSelect) {
  const handleRegistrationPlanChange = function() {
    const nextPlan = String(this.value || '').toLowerCase();
    if (!nextPlan) {
      selectedPlan = null;
      syncPlanSelectors('');
    ...
    syncPlanSelectors(nextPlan);
    updateSelectedPlanUI(nextPlan);
  };
  registrationPlanSelect.addEventListener('change', handleRegistrationPlanChange);
  registrationPlanSelect.addEventListener('input', handleRegistrationPlanChange);
}
