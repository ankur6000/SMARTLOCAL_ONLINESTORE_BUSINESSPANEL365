...
document.getElementById('login-form').addEventListener('submit', async function(e) {
  ...
  try {
    ...
    await updateDashboard({ refreshRates: true });
    await refreshCommandCenterData({ silent: true, source: 'login', refreshRates: false, syncPortalState: true });
...
function buildOrderCourierLabel(order = {}) {
  ...
  const provider = formatProviderLabel(order?.courier?.provider || 'smartlocal');
  const awb = String(order?.courier?.awb || '').trim();
  const status = order?.courier?.status || (order?.status === 'Dispatched' ? 'Dispatched' : 'Pending Dispatch');
  ...
  tbody.innerHTML = rows.length
    ? rows.map((row) => `
      <tr>
        ...
        <td>${formatRupees(row.amount || 0)}</td>
      </tr>
    `).join('')
    : '<tr><td colspan="7">No live statement entries found.</td></tr>';
}
...
async function handleStatementExport(format = 'pdf') {
  ...
  try {
    ...
    const result = await window.smartlocalDownloadStatement(format, {
      ...
      title: `${currentUser.business?.name || currentUser.name || 'SmartLocal'} Live Statement`
    });
...
async function renderCustomersTab(registrationData = null) {
  ...
  if (remote && Array.isArray(remote.items)) {
    ...
    document.getElementById('customer-growth').textContent = formatDeltaText(currentMonthRegistrations, previousMonthRegistrations);
    document.getElementById('returning-customers').textContent = countReturningCustomers(getLocalOrders());
  ...
  document.getElementById('returning-customers').textContent = 0;
...
function updatePortfolioSummary(record) {
  ...
  if (stopLossInput) stopLossInput.value = normalizedRecord.stopLossValue > 0 ? normalizedRecord.stopLossValue : '';
  if (takeProfitInput) takeProfitInput.value = normalizedRecord.takeProfitValue > 0 ? normalizedRecord.takeProfitValue : '';
  if (autoProtectInput) autoProtectInput.value = normalizedRecord.autoWithdrawEnabled ? 'on' : 'off';
...
function buildPortfolioWithdrawalRecord(currentRecord, requestedAmount = 0, mode = 'full') {
  ...
  const fullWithdraw = mode === 'full' || requestedAmount >= currentValue;
  const withdrawValue = fullWithdraw ? currentValue : roundPortalAmount(requestedAmount);
...
function saveCourierOrder(order) {
  upsertLocalCollection('courierOrders', 'awb', order, 20);
...
function getLocalCouriers() {
  return Array.isArray(dashboardLiveData.couriers) ? dashboardLiveData.couriers : [];
}
...
function ensureTabExportBars() {
  document.querySelectorAll('.tab-content').forEach((tab) => {
    if (tab.id === 'tab-statements') return;
...
async function handlePortfolioWithdraw(mode = 'full') {
  ...
  const effectiveMode = mode === 'partial' && requestedAmount < liveCurrentValue ? 'partial' : 'full';
  const withdrawnRecord = buildPortfolioWithdrawalRecord(currentRecord, requestedAmount, effectiveMode);
  try {
...
async function handleAdminOrderSubmit(event) {
  ...
  try {
    ...
    if (summary) {
      summary.innerHTML = `Order <strong>${escapeHtml(savedOrder.orderId)}</strong> saved for <strong>${escapeHtml(savedOrder.payment || 'UPI')}</strong>. Invoice is ready and courier is queued as <strong>${escapeHtml(savedOrder.courier?.status || 'Pending Dispatch')}</strong> until cutoff ${formatCutoffLabel((data.settings || businessSettings).bookingCutoff)}.`;
    }
    ...
    showToast('Order created and queued for automatic courier dispatch.', 'success');
  } catch (error) {
    const summary = document.getElementById('order-summary');
...
function downloadBulkOrderSampleExcel() {
  const headers = ['customerName', 'customerPhone', 'customerEmail', 'customerAddress', 'items', 'gstPercent', 'payment', 'originPincode', 'destinationPincode', 'weightKg', 'declaredValue', 'provider', 'orderDate'];
...
function buildBulkOrderPayload(row, rowIndex, currentUser) {
  const items = parseAdminOrderItems(row.items || '');
  if (!items.length) throw new Error('items column is missing or invalid');
...
async function handleBulkOrderUpload() {
  ...
  try {
    ...
    showToast(`Bulk upload finished. ${data.createdCount || 0} orders queued for dispatch.`, 'success');
  } catch (error) {
