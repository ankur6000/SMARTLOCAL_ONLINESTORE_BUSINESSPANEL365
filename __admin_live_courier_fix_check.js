...
function normalizeDashboardOrder(order) {
  ...
  const normalizedStatus = /cancel/i.test(rawStatus)
    ...
    : (/deliver|complete/i.test(rawStatus) || normalizedCourierStatus === 'Delivered')
      ? 'Delivered'
      : (/(dispatch|ship|transit|out for)/i.test(rawStatus) || ['Dispatched', 'Shipped', 'In Transit', 'Out for Delivery'].includes(normalizedCourierStatus))
        ? 'Dispatched'
  ...
  normalized.dispatchStatus = normalizedStatus === 'Delivered'
    ? 'Delivered'
    : (normalizedStatus === 'Dispatched' ? 'Dispatched' : 'Pending Dispatch');
