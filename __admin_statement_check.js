...
function normalizeDashboardCourier(courier = {}) {
  ...
  return {
    ...
    status: String(courier?.status || 'Pending Dispatch').trim() || 'Pending Dispatch',
...
function mergeCourierCollections(couriersInput = [], ordersInput = []) {
  ...
  return sortAdminCourierRows(Array.from(merged.values())).slice(0, 200);
}
...
function setLiveCollection(storageKey, items) {
  ...
  if (key === 'orders') {
    ...
    dashboardLiveData.couriers = mergeCourierCollections(dashboardLiveData.couriers, dashboardLiveData.orders);
    return dashboardLiveData.orders;
  }
  if (key === 'couriers') {
    dashboardLiveData.couriers = mergeCourierCollections(items, dashboardLiveData.orders);
...
function getLocalCouriers() {
  return mergeCourierCollections(dashboardLiveData.couriers, dashboardLiveData.orders);
}
...
function upsertLocalCollection(storageKey, idKey, record, limit = 200) {
  ...
  if (key === 'couriers') {
    dashboardLiveData.couriers = mergeCourierCollections(filtered.slice(0, limit), dashboardLiveData.orders);
    return dashboardLiveData.couriers;
  }
