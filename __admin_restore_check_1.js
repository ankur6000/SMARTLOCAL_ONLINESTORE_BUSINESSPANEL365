...
async function fetchRegistrations(limit = 6) {
  try {
...
async function renderCustomersTab() {
  ...
  const remote = await fetchRegistrations(6);
  ...
  document.getElementById('returning-customers').textContent = Math.round(count * 0.68) || 0;
  if (!tbody) return;
  tbody.innerHTML = users.slice(0, 6).map(user => `
    <tr>
      <td>${user.name || 'N/A'}</td>
      <td>${user.email || '-'}</td>
      <td>${user.business?.name || 'N/A'}</td>
...
async function syncOverviewPortal(orders, totalRevenue, totalOrders, yearOrders, averageOrder, currentUser, planLabel) {
  ...
  const latestAwb = courierOrders[0]?.awb || JSON.parse(localStorage.getItem('lastAWB') || 'null') || 'N/A';
...
function getPortfolioBounds(amount) {
  const invested = Math.max(Number(amount) || 0, 100);
  return {
    invested,
...
function normalizePortfolioRecord(record = {}) {
  const invested = Math.max(Number(record.amountInvested) || 0, 100);
  const currentValue = clampPortfolioValue(record.currentValue ?? invested, invested);
  const profitLoss = currentValue - invested;
  const returnPercent = Number((((currentValue - invested) / invested) * 100).toFixed(1));
  const trend = profitLoss < 0 ? 'loss' : 'profit';
  const points = Array.isArray(record.points) && record.points.length
  ...
  return {
    ...
    lastUpdatedAt: new Date(record.lastUpdatedAt || Date.now()).toISOString(),
...
function resizePortfolioCanvas(canvas) {
  ...
  const height = window.innerWidth < 640 ? 360 : window.innerWidth < 980 ? 460 : 560;
...
function drawPortfolioGraph(points, trend, investedAmount = 0, currentValue = 0) {
  ...
  if (!canvas || !canvas.getContext || !Array.isArray(points) || !points.length) return;

  const { ctx, width, height } = resizePortfolioCanvas(canvas);
  const padding = { top: 62, right: 28, bottom: 48, left: 70 };
...
function updatePortfolioSummary(record) {
  ...
  const trendClass = normalizedRecord.trend === 'loss' ? 'red' : 'green';
  const liveColor = normalizedRecord.trend === 'loss' ? '#ffb0bb' : '#86ffd0';
  ...
  if (marketModeEl) {
    marketModeEl.textContent = normalizedRecord.trend === 'loss' ? 'Red Wave' : 'Green Wave';
    marketModeEl.style.color = liveColor;
  }
  if (asOfLiveEl) asOfLiveEl.textContent = lastUpdatedText;
  if (statusPill) {
    statusPill.textContent = normalizedRecord.trend === 'loss' ? 'LIVE LOSS' : 'LIVE PROFIT';
    statusPill.className = `market-status-pill ${normalizedRecord.trend === 'loss' ? 'loss' : 'profit'}`;
  }
  if (liveBadge) {
    liveBadge.textContent = normalizedRecord.trend === 'loss' ? 'Loss / Live' : 'Profit / Live';
    liveBadge.className = `badge-sm ${normalizedRecord.trend === 'loss' ? 'loss-badge' : 'profit-badge'}`;
  }
  if (noteEl) {
    const movement = normalizedRecord.trend === 'loss'
...
async function loadPortfolioDashboard() {
  const items = await fetchPortfolioInvestments(20);
  if (!items.length) {
    updatePortfolioSummary(normalizePortfolioRecord({
      portfolioName: 'SmartLocal Live Market',
      amountInvested: 10000,
      currentValue: 10850,
      trend: 'profit',
      lastUpdatedAt: new Date().toISOString(),
      points: generatePortfolioGraphPoints(10000, 10850)
    }));
    return;
  }
  updatePortfolioSummary(normalizePortfolioRecord(items[0]));
}
...
async function handlePortfolioSubmit(event) {
  ...
  try {
    await savePortfolioInvestment(record);
    updatePortfolioSummary(record);
    startPortfolioTicker();
...
async function updateDashboard() {
  const currentUser = getSafeCurrentUser() || {};
  const orders = await fetchDashboardOrders(50);
  await fetchDashboardCollection('/api/couriers', 'courierOrders', 50);
  await fetchDashboardCollection('/api/complaints', 'complaints', 50);
...
function updateCourierTable() {
  ...
  tbody.innerHTML = courierOrders.map(order => {
    const providerLabel = (order.provider || 'smartlocal');
    return `
      <tr>
        <td>${order.awb}</td>
        <td>${providerLabel.replace(/([A-Z])/g, ' $1').trim()}</td>
...
function generateAWB() {
  ...
  const costMultiplier = service === 'express' ? 1.5 : service === 'same-day' ? 2.2 : 1;
  const cost = Math.max(80, Math.round((weight * 25 + value * 0.02) * costMultiplier));
  const awb = `AWB${Date.now().toString().slice(-8)}`;
  const order = {
    awb,
    provider,
...
function startPortfolioTicker() {
  ...
  portfolioLiveTimer = setInterval(() => {
    const dashboard = document.getElementById('dashboard-section');
    if (!dashboard || dashboard.style.display === 'none') return;

    if (!portfolioLiveState || !(Number(portfolioLiveState.amountInvested) > 0)) return;

    const currentRecord = normalizePortfolioRecord(portfolioLiveState);
