...
function normalizePortalState(raw = {}) {
  ...
  return {
    ...
    robertLastProvider: ['local', 'gemini', 'groq'].includes(String(raw?.robertLastProvider || '').trim().toLowerCase()) ? String(raw.robertLastProvider).trim().toLowerCase() : '',
...
function normalizeDashboardNotification(notification = {}) {
  const publishedAt = String(notification?.publishedAt || notification?.createdAt || new Date().toISOString()).trim() || new Date().toISOString();
  const priority = ['info', 'success', 'warning', 'alert'].includes(String(notification?.priority || '').trim().toLowerCase())
    ? String(notification.priority).trim().toLowerCase()
    : 'info';
  const status = ['active', 'archived'].includes(String(notification?.status || '').trim().toLowerCase())
    ? String(notification.status).trim().toLowerCase()
    : 'active';
  return {
    ...notification,
    id: String(notification?.id || notification?._id || '').trim(),
    businessUserId: String(notification?.businessUserId || '').trim(),
    businessName: String(notification?.businessName || '').trim(),
...
function normalizeDashboardComplaint(complaint = {}) {
  const rawStatus = String(complaint?.status || '').trim().toLowerCase();
  const status = rawStatus.includes('reject')
    ? 'Rejected'
    : ((rawStatus.includes('closed') && rawStatus.includes('resolved')) || rawStatus.includes('auto close'))
      ? 'Closed / Resolved'
      : rawStatus.includes('resolve')
        ...
        : rawStatus.includes('approve')
          ...
          : rawStatus.includes('progress')
            ? 'In Progress'
            : 'Open';
  const rawPriority = String(complaint?.priority || '').trim().toLowerCase();
  const priority = rawPriority.includes('high')
    ? 'High'
  ...
  return {
    ...complaint,
    id: String(complaint?.id || complaint?._id || '').trim(),
    name: String(complaint?.name || '').trim(),
    phone: String(complaint?.phone || '').trim(),
    category: String(complaint?.category || 'Other').trim() || 'Other',
    priority,
    details: String(complaint?.details || '').trim(),
    status,
    date: String(complaint?.date || complaint?.createdAt || new Date().toISOString()).trim() || new Date().toISOString(),
    automatedRemarks: Array.isArray(complaint?.automatedRemarks) ? complaint.automatedRemarks : [],
    analysisSummary: String(complaint?.analysisSummary || '').trim(),
    analysisSeverity: String(complaint?.analysisSeverity || '').trim(),
    analysisAction: String(complaint?.analysisAction || '').trim()
  };
}

function isComplaintFinalStatus(status = '') {
  return ['Resolved', 'Rejected', 'Closed / Resolved'].includes(String(status || '').trim());
}

function complaintStatusBadgeClass(status = '') {
  const clean = String(status || '').trim();
  if (clean === 'Rejected') return 'complaint-status-rejected';
  if (clean === 'Approved') return 'complaint-status-approved';
  if (clean === 'Resolved' || clean === 'Closed / Resolved') return 'complaint-status-resolved';
  if (clean === 'In Progress') return 'complaint-status-progress';
...
function setLiveCollection(storageKey, items) {
  ...
  if (key === 'notifications') {
    dashboardLiveData.notifications = (Array.isArray(items) ? items : []).map(normalizeDashboardNotification);
    return dashboardLiveData.notifications;
  }
  if (key === 'complaints') {
    dashboardLiveData.complaints = (Array.isArray(items) ? items : []).map(normalizeDashboardComplaint);
    return dashboardLiveData.complaints;
  }
...
function populateYearStatement(orders) {
  ...
  tbody.innerHTML = orders.map(order => {
    ...
    const customerName = order?.customerName || order?.party || order?.customer || '-';
    ...
    return `
      <tr>
        <td class="statement-date-cell">${escapeHtml(formatAdminDate(order.date))}</td>
        <td>${escapeHtml(order.orderId || '-')}</td>
        <td>${escapeHtml(customerName)}</td>
        <td>${escapeHtml(itemSummary)}</td>
        <td>${formatRupees(order.amount)}</td>
        <td><span class="statement-entry-tag ${statusClass}">${escapeHtml(order.status || '-')}</span></td>
...
function renderOverviewLiveFeed(orders) {
  ...
  const markup = events.length ? events.map(event => `<li class="activity-feed-item">${event}</li>`).join('') : '<li class="activity-feed-item">No live events yet.</li>';
  if (feed) feed.innerHTML = markup;
  const overviewTabFeed = document.getElementById('overview-tab-live-feed');
  if (overviewTabFeed) overviewTabFeed.innerHTML = markup;
}
...
async function syncOverviewPortal(orders, totalRevenue, totalOrders, yearOrders, averageOrder, currentUser, planLabel, registrationData = null) {
  ...
  const tabSummaryRevenue = document.getElementById('overview-tab-summary-revenue');
  const tabSummaryAverage = document.getElementById('overview-tab-summary-average-order');
  const tabSummaryCouriers = document.getElementById('overview-tab-summary-couriers');
  const tabSummaryComplaints = document.getElementById('overview-tab-summary-complaints');
  const tabSummaryMarketReturn = document.getElementById('overview-tab-summary-market-return');
  const tabSummaryLatestAwb = document.getElementById('overview-tab-summary-latest-awb');
  const overviewCopy = document.getElementById('overview-copy');
  const dashboardMeta = document.getElementById('dashboard-meta');
  const overviewTabMeta = document.getElementById('overview-tab-meta');
  const overviewTabCopy = document.getElementById('overview-tab-copy');
  ...
  if (tabSummaryRevenue) tabSummaryRevenue.textContent = formatRupees(totalRevenue);
  if (tabSummaryAverage) tabSummaryAverage.textContent = formatRupees(averageOrder);
  if (tabSummaryCouriers) tabSummaryCouriers.textContent = courierOrders.length;
  if (tabSummaryComplaints) tabSummaryComplaints.textContent = complaints.length;
  ...
  if (tabSummaryMarketReturn) {
    tabSummaryMarketReturn.textContent = `${marketReturn.toFixed(1)}%`;
    tabSummaryMarketReturn.style.color = marketReturn < 0 ? '#ff6a7f' : '#ffd86b';
  }
  if (summaryLatestAwb) summaryLatestAwb.textContent = latestAwb;
  if (tabSummaryLatestAwb) tabSummaryLatestAwb.textContent = latestAwb;
  ...
  if (overviewTabCopy) {
    overviewTabCopy.textContent = `${businessName} is synchronized with ${totalOrders} orders, ${courierOrders.length} courier bookings, ${complaints.length} complaints, ${customers} customers, and ${formatRupees(marketTransferRevenue)} protected market transfers in revenue.`;
  }
  ...
  if (overviewTabMeta) {
    overviewTabMeta.textContent = `Plan: ${planLabel} | Customers: ${customers} | Latest AWB: ${latestAwb} | Updated: ${formatStableAdminTime(new Date())}`;
  }
...
function renderSegmentPlanControls(user = getSafeCurrentUser()) {
  ...
  const overviewBadge = document.getElementById('overview-tab-segment-badge');
  const overviewIcon = document.getElementById('overview-tab-segment-icon');
  const overviewName = document.getElementById('overview-tab-segment-name');
  if (overviewBadge) overviewBadge.className = `profile-segment-badge overview-hub-badge ${currentPlan}-segment`;
  if (overviewIcon) overviewIcon.textContent = currentDetails.name.charAt(0);
  if (overviewName) overviewName.textContent = currentDetails.name;
}
...
async function updateDashboard(options = {}) {
  ...
  await fetchDashboardNotifications(200);
  updateCourierTable();
  ...
  if (document.getElementById('overview-tab-summary-orders')) document.getElementById('overview-tab-summary-orders').textContent = totalOrders;
  if (document.getElementById('overview-tab-summary-year-transactions')) document.getElementById('overview-tab-summary-year-transactions').textContent = yearOrders;
  if (document.getElementById('overview-tab-summary-revenue')) document.getElementById('overview-tab-summary-revenue').textContent = formatRupees(totalRevenue);
  if (document.getElementById('overview-tab-summary-average-order')) document.getElementById('overview-tab-summary-average-order').textContent = formatRupees(averageOrder);
  ...
  if (document.getElementById('overview-tab-copy')) document.getElementById('overview-tab-copy').textContent = currentUser.business?.name ? `${currentUser.business.name} is synced with live bookings, revenue, courier, complaints, and yearly business metrics.` : 'All core business overview, revenue, courier, complaint, and activity metrics are shown here in one place.';
  ...
  const overviewTabCard = document.getElementById('overview-tab-card');
  ...
  if (segmentBadge) {
    ...
    if (overviewTabCard) overviewTabCard.className = 'dash-metric overview-panel overview-hub-card';

    if (planTier === 'silver') {
      segmentBadge.classList.add('silver-segment');
      if (overviewCard) overviewCard.classList.add('silver-segment');
      if (overviewTabCard) overviewTabCard.classList.add('silver-segment');
      ...
      if (overviewTabCard) overviewTabCard.classList.add('platinum-segment');
      ...
      if (overviewTabCard) overviewTabCard.classList.add('gold-segment');
  ...
  if (document.getElementById('overview-tab-summary-couriers')) document.getElementById('overview-tab-summary-couriers').textContent = getLocalCouriers().length;
  if (document.getElementById('overview-tab-summary-complaints')) document.getElementById('overview-tab-summary-complaints').textContent = getLocalComplaints().length;
  ...
  renderNotificationsTab(getLocalNotifications());
  updateCourierTable();
  renderCourierSummary();
...
async function saveComplaintToServer(complaint) {
  try {
    ...
    const data = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(data?.error || 'Unable to save complaint right now.');
    return normalizeDashboardComplaint(data?.record || complaint);
  } catch (err) {
    throw err;
  }
}

async function updateComplaintStatusOnServer(complaintId, status) {
  const currentUser = currentBusinessUser();
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data?.error || 'Unable to update complaint status right now.');
  return normalizeDashboardComplaint(data?.record || { id: complaintId, status });
}
...
function renderComplaintTable() {
  ...
  tbody.innerHTML = complaints.map((complaint) => `
    <tr>
      ...
      <td><span class="complaint-status-pill ${complaintStatusBadgeClass(complaint.status)}">${escapeHtml(complaint.status)}</span></td>
      <td>${escapeHtml(formatAdminDate(complaint.date))}</td>
      <td>
        <div class="complaint-action-group">
          ${isComplaintFinalStatus(complaint.status) ? `<span class="badge-sm ${complaint.status === 'Rejected' ? 'complaint-status-rejected' : 'complaint-status-resolved'}">${escapeHtml(complaint.status)}</span>` : `
            <button type="button" class="glow-btn btn-outline-light complaint-action-btn" data-id="${escapeHtml(complaint.id)}" data-status="Approved">Approve</button>
            <button type="button" class="glow-btn btn-outline-light complaint-action-btn" data-id="${escapeHtml(complaint.id)}" data-status="Resolved">Resolved</button>
            <button type="button" class="glow-btn btn-outline-light complaint-action-btn" data-id="${escapeHtml(complaint.id)}" data-status="Rejected">Rejected</button>
          `}
        </div>
      </td>
    </tr>
...
async function handleAdminComplaintTableClick(event) {
  const button = event.target.closest('.complaint-action-btn');
  if (!button) return;
  const complaintId = String(button.dataset.id || '').trim();
  const nextStatus = String(button.dataset.status || '').trim();
  if (!complaintId || !nextStatus) return;
  try {
    button.disabled = true;
    const savedComplaint = await updateComplaintStatusOnServer(complaintId, nextStatus);
    upsertLocalCollection('complaints', 'id', savedComplaint, 200);
    renderComplaintTable();
    renderComplaintStats();
    await refreshCommandCenterData({ silent: true, source: 'complaint-status' });
    showToast(`Complaint ${complaintId} updated to ${savedComplaint.status}.`, 'success');
  } catch (error) {
    showToast(error.message || 'Unable to update complaint status.', 'error');
  } finally {
    button.disabled = false;
  }
}

async function handleAdminComplaintSubmit(event) {
  event.preventDefault();
  const businessSettings = currentBusinessSettings();
  const complaint = normalizeDashboardComplaint({
  ...
  try {
    const savedComplaint = await saveComplaintToServer(complaint);
    upsertLocalCollection('complaints', 'id', savedComplaint, 100);
    const summary = document.getElementById('complaint-summary');
    if (summary) {
      summary.innerHTML = `<div class="confirmation-card"><strong>Complaint ID:</strong> ${escapeHtml(savedComplaint.id)}<br><strong>Status:</strong> ${escapeHtml(savedComplaint.status)}<br><strong>Priority:</strong> ${escapeHtml(savedComplaint.priority)}<br><strong>Cutoff:</strong> ${escapeHtml(formatCutoffLabel(businessSettings.bookingCutoff))}</div>`;
    }
    document.getElementById('complaint-form')?.reset();
    renderComplaintTable();
    renderComplaintStats();
    await refreshCommandCenterData({ silent: true, source: 'complaint-save' });
    showToast(
      savedComplaint.status === 'Closed / Resolved'
        ? `Complaint saved and auto-closed after cutoff ${formatCutoffLabel(businessSettings.bookingCutoff)}.`
        : 'Complaint saved successfully.',
      'success'
    );
  } catch (error) {
    showToast(error.message || 'Unable to save complaint right now.', 'error');
  }
}
...
function getLocalComplaints() {
  return (Array.isArray(dashboardLiveData.complaints) ? dashboardLiveData.complaints : [])
    .map(normalizeDashboardComplaint)
...
function getLocalNotifications() {
  return (Array.isArray(dashboardLiveData.notifications) ? dashboardLiveData.notifications : [])
...
function upsertLocalCollection(storageKey, idKey, record, limit = 200) {
  ...
  if (key === 'notifications') {
    dashboardLiveData.notifications = filtered.slice(0, limit).map(normalizeDashboardNotification);
    return dashboardLiveData.notifications;
  }
  if (key === 'complaints') {
    dashboardLiveData.complaints = filtered.slice(0, limit).map(normalizeDashboardComplaint);
    return dashboardLiveData.complaints;
  }
...
function ensureTabExportBars() {
  document.querySelectorAll('.tab-content').forEach((tab) => {
    if (tab.id === 'tab-statements' || tab.id === 'tab-metals' || tab.id === 'tab-notifications') return;
...
async function refreshCommandCenterData(options = {}) {
  ...
  try {
    ...
    try {
      notifications = await fetchDashboardNotifications(200);
    } catch (error) {
...
async function fetchDashboardNotifications(limit = 100) {
  const items = await fetchDashboardCollection('/api/notifications', 'notifications', limit);
  return Array.isArray(items) ? items.map(normalizeDashboardNotification) : [];
}
...
async function saveNotificationToServer(notification) {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data?.error || 'Failed to publish notification.');
  return normalizeDashboardNotification(data?.record || notification);
}

async function updateNotificationOnServer(notificationId, payload = {}) {
  const currentUser = currentBusinessUser();
  ...
  return normalizeDashboardNotification(data?.record || payload);
}

function notificationPriorityLabel(priority = 'info') {
  const labels = {
    info: 'Info',
    success: 'Success',
    warning: 'Warning',
...
function renderNotificationsTab(notificationsInput = getLocalNotifications()) {
  const notifications = (Array.isArray(notificationsInput) ? notificationsInput : []).map(normalizeDashboardNotification)
    .sort((left, right) => new Date(right.publishedAt || 0) - new Date(left.publishedAt || 0));
  ...
  const currentUser = currentBusinessUser();
  const businessField = document.getElementById('notification-business-label');
  const activeEl = document.getElementById('notification-active-count');
  const liveStatusEl = document.getElementById('notification-live-status');
  const totalEl = document.getElementById('notification-total-count');
  const latestTimeEl = document.getElementById('notification-latest-time');
  const summaryNoteEl = document.getElementById('notification-summary-note');

  if (businessField) businessField.value = currentUser?.business?.name || currentUser?.name || '';
  if (activeEl) activeEl.textContent = String(activeCount);
  if (liveStatusEl) liveStatusEl.textContent = activeCount ? `${activeCount} live on homepage` : 'Homepage feed waiting';
  if (totalEl) totalEl.textContent = String(totalCount);
  if (latestTimeEl) latestTimeEl.textContent = latest ? formatAdminDate(latest.publishedAt) : 'No publish yet';
  if (summaryNoteEl) summaryNoteEl.textContent = totalCount
    ? `${totalCount} notifications are stored in MongoDB. ${activeCount} notification${activeCount === 1 ? '' : 's'} are visible on the homepage feed.`
    : 'Use this space for short, important notices only.';

  if (!tbody) return;
  tbody.innerHTML = notifications.length
    ? notifications.map((item) => `
      <tr>
        <td>${escapeHtml(formatAdminDate(item.publishedAt))}</td>
        <td>${escapeHtml(item.title || '-')}</td>
        <td class="notification-message-cell">${escapeHtml(item.message || '-')}</td>
        <td><span class="notification-priority-badge notification-priority-${escapeHtml(item.priority)}">${escapeHtml(notificationPriorityLabel(item.priority))}</span></td>
        <td>${escapeHtml(item.status === 'active' ? 'Live' : 'Archived')}</td>
        <td><button type="button" class="glow-btn btn-outline-light notification-status-btn" data-id="${escapeHtml(item.id)}" data-status="${escapeHtml(item.status === 'active' ? 'archived' : 'active')}">${item.status === 'active' ? 'Archive' : 'Show Live'}</button></td>
      </tr>
    `).join('')
    : '<tr><td colspan="6">No notifications published yet.</td></tr>';
}
...
async function handleNotificationSubmit(event) {
  ...
  const currentUser = currentBusinessUser();
  if (!currentUser?.userid) {
    showToast('Please log in before publishing notifications.', 'error');
    return;
  }
  const title = String(document.getElementById('notification-title')?.value || '').trim();
  const message = String(document.getElementById('notification-message')?.value || '').trim();
  const priority = String(document.getElementById('notification-priority')?.value || 'info').trim().toLowerCase();
  const status = String(document.getElementById('notification-status')?.value || 'active').trim().toLowerCase();
  const statusNote = document.getElementById('notification-publish-status');
  if (!title || !message) {
    showToast('Notification title and message are required.', 'error');
    return;
  }
  try {
    if (statusNote) statusNote.textContent = 'Publishing notification to MongoDB...';
    const savedNotification = await saveNotificationToServer({
      businessUserId: currentUser.userid,
      businessName: currentUser.business?.name || currentUser.name || '',
      title,
      message,
    ...
    upsertLocalCollection('notifications', 'id', savedNotification, 200);
    renderNotificationsTab(getLocalNotifications());
    document.getElementById('notification-form')?.reset();
    const businessField = document.getElementById('notification-business-label');
    if (businessField) businessField.value = currentUser.business?.name || currentUser.name || '';
    if (statusNote) statusNote.textContent = `Notification published${savedNotification.status === 'active' ? ' to the live homepage feed' : ' and saved in MongoDB'}.`;
    showToast('Notification published successfully.', 'success');
  } catch (error) {
    if (statusNote) statusNote.textContent = 'Notification could not be saved right now.';
    showToast(error.message || 'Notification could not be published.', 'error');
  }
}

async function handleNotificationTableClick(event) {
  const button = event.target.closest('.notification-status-btn');
  if (!button) return;
  const notificationId = String(button.dataset.id || '').trim();
  const nextStatus = String(button.dataset.status || '').trim().toLowerCase();
  if (!notificationId || !nextStatus) return;
  try {
    const savedNotification = await updateNotificationOnServer(notificationId, { status: nextStatus });
    upsertLocalCollection('notifications', 'id', savedNotification, 200);
    renderNotificationsTab(getLocalNotifications());
    showToast(`Notification ${nextStatus === 'active' ? 'moved live' : 'archived'}.`, 'success');
  } catch (error) {
...
function parseAdminOrderItems(raw) {
  return String(raw || '')
    ...
    .map((line) => {
      ...
      const naturalMatch = line.match(/^(.*?)\s*x\s*(\d+)\s*(?:@|at|rs\.?|inr|\u20B9)?\s*([\d.,]+)$/i);
      if (naturalMatch) {
        return {
          name: String(naturalMatch[1] || '').trim(),
          qty: Math.max(Math.round(Number(naturalMatch[2]) || 0), 1),
          price: Math.max(roundPortalAmount(String(naturalMatch[3] || '').replace(/,/g, '')), 0),
          hsn: ''
        };
      }
      return null;
...
function downloadBulkOrderSampleExcel() {
  const headers = ['Customer Name', 'Phone Number', 'Email', 'Delivery Address', 'Item Name 1', 'Qty 1', 'Price 1', 'Item Name 2', 'Qty 2', 'Price 2', 'GST %', 'Payment Type', 'Pickup Pincode', 'Delivery Pincode', 'Weight (Kg)', 'Order Value', 'Courier Provider', 'Order Date'];
...
function normalizeBulkHeader(header) {
  ...
  const indexedMatch = clean.match(/(\d+)$/);
  if (indexedMatch) {
    const index = indexedMatch[1];
    const stem = clean.slice(0, -index.length);
    if (/(?:item|product|service|description|desc|particular|sku|name)/.test(stem) && !/(?:customer|party|client|buyer|seller|company)/.test(stem)) {
      return `itemName${index}`;
    }
    if (/(?:qty|quantity|unit|units|piece|pieces|pcs|count|nos|no|required|reqqty|orderedqty|orderqty)/.test(stem)) {
      return `qty${index}`;
    }
    if (/(?:price|rate|amount|cost|mrp|value|saleprice|sellingprice|unitprice|unitrate|lineamount|linevalue|itemamount)/.test(stem)) {
      return `price${index}`;
    }
    if (/(?:hsn|sac)/.test(stem)) {
      return `hsn${index}`;
    }
  }
  const map = {
    ...
    pickupzipcode: 'originPincode',
    pickupzip: 'originPincode',
    sourcepincode: 'originPincode',
    ...
    deliveryzip: 'destinationPincode',
    deliveryzipcode: 'destinationPincode',
    ...
    orderamount: 'declaredValue',
    invoiceamount: 'declaredValue',
    billamount: 'declaredValue',
    provider: 'provider',
  ...
  if ((/(?:pickup|origin|from|sender|source)/.test(clean)) && (/(?:pin|pincode|postcode|postal|zipcode|zip|code)/.test(clean))) return 'originPincode';
  if ((/(?:delivery|destination|to|receiver|drop)/.test(clean)) && (/(?:pin|pincode|postcode|postal|zipcode|zip|code)/.test(clean))) return 'destinationPincode';
  if (/(?:mobile|phone|contact|whatsapp)/.test(clean)) return 'customerPhone';
  if (/email/.test(clean)) return 'customerEmail';
  if (/(?:address|addr|location)/.test(clean)) return 'customerAddress';
  if (/(?:payment|paymode|paymentmethod)/.test(clean)) return 'payment';
  if (/(?:gst|tax)/.test(clean)) return 'gstPercent';
  if (/(?:weight|chargeableweight|deadweight|kg)/.test(clean)) return 'weightKg';
  if (/(?:courier|carrier|provider|partner|shipper)/.test(clean)) return 'provider';
  if (/(?:declaredvalue|ordervalue|invoicevalue|shipmentvalue|orderamount|billamount|declaredamount)/.test(clean)) return 'declaredValue';
  if (/(?:itemname|productname|description|itemdescription|productdescription|particular|serviceitem|servicename)/.test(clean)) return 'itemName';
  if (/(?:items|orderitems|itemlist|productlist|productdetails|itemdetails|particulars)/.test(clean)) return 'items';
  if (/(?:qty|quantity|units|unit|pieces|piece|pcs|count|nos|requiredqty|orderedqty)/.test(clean)) return 'qty';
  if (/(?:price|rate|amount|cost|mrp|unitprice|sellingprice|itemprice|productprice|saleprice|unitrate|lineamount|itemamount)/.test(clean)) return 'price';
  if (/(?:hsn|sac)/.test(clean)) return 'hsn';
  return header;
}
...
function splitBulkCellValues(value) {
  return String(value || '')
    .split(/\r?\n|;/)
    .map((part) => part.trim())
    .filter(Boolean)
    .flatMap((part) => part.includes(',') ? part.split(',').map((chunk) => chunk.trim()).filter(Boolean) : [part]);
}

function buildBulkItemsFromParallelColumns(row = {}) {
  const names = splitBulkCellValues(row.items || row.itemName || row.productName || row.item || row.product || row.description || row.itemDescription || row.productDescription || row.particular || '');
  const qtys = splitBulkCellValues(row.qty || row.quantity || row.units || row.unit || row.pieces || row.pcs || row.count || row.requiredQty || '');
  const prices = splitBulkCellValues(row.price || row.rate || row.unitPrice || row.itemPrice || row.amount || row.itemAmount || row.cost || '');
  const hsns = splitBulkCellValues(row.hsn || row.sac || '');
  if (names.length > 1 && names.length === qtys.length && names.length === prices.length) {
    return names.map((name, index) => ({
      name: String(name || '').trim(),
      qty: Math.max(Math.round(Number(qtys[index] || 0) || 0), 0),
      price: Math.max(roundPortalAmount(prices[index] || 0), 0),
      hsn: String(hsns[index] || '').trim()
    })).filter((item) => item.name && item.qty > 0);
  }
  return [];
}
...
function buildBulkItemsFromRow(row = {}) {
  const directItems = parseAdminOrderItems(row.items || row.orderItems || row.itemList || row.productList || row.productDetails || row.itemDetails || '');
  if (directItems.length) return directItems;
  const listItems = buildBulkItemsFromParallelColumns(row);
  ...
  seedPrimary.name = String(row.itemName || row.productName || row.item || row.product || row.items || row.description || row.itemDescription || row.productDescription || row.particular || '').trim();
  seedPrimary.qty = String(row.qty || row.quantity || row.units || row.unit || row.pieces || row.pcs || row.count || row.requiredQty || '').trim();
  seedPrimary.price = String(row.price || row.rate || row.unitPrice || row.itemPrice || row.amount || row.itemAmount || row.cost || '').trim();
  seedPrimary.hsn = String(row.hsn || row.sac || '').trim();

  Object.entries(row || {}).forEach(([key, value]) => {
    const textValue = String(value || '').trim();
    if (!textValue) return;
    let match = key.match(/^itemName(\d+)$/i) || key.match(/^productName(\d+)$/i) || key.match(/^itemDescription(\d+)$/i) || key.match(/^productDescription(\d+)$/i) || key.match(/^description(\d+)$/i) || key.match(/^particular(?:s)?(\d+)$/i) || key.match(/^items?(\d+)$/i) || key.match(/^item(\d+)$/i) || key.match(/^product(\d+)$/i) || key.match(/^name(\d+)$/i);
    if (match) {
      ensureGroup(match[1]).name = textValue;
      return;
    }
    match = key.match(/^qty(\d+)$/i) || key.match(/^quantity(\d+)$/i) || key.match(/^units?(\d+)$/i) || key.match(/^pieces?(\d+)$/i) || key.match(/^pcs(\d+)$/i) || key.match(/^count(\d+)$/i) || key.match(/^nos(\d+)$/i) || key.match(/^requiredQty(\d+)$/i);
    if (match) {
      ensureGroup(match[1]).qty = textValue;
      return;
    }
    match = key.match(/^price(\d+)$/i) || key.match(/^rate(\d+)$/i) || key.match(/^unitPrice(\d+)$/i) || key.match(/^itemPrice(\d+)$/i) || key.match(/^amount(\d+)$/i) || key.match(/^itemAmount(\d+)$/i) || key.match(/^cost(\d+)$/i);
    if (match) {
      ensureGroup(match[1]).price = textValue;
      return;
    }
    match = key.match(/^hsn(\d+)$/i) || key.match(/^sac(\d+)$/i);
    if (match) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('complaint-table')?.addEventListener('click', handleAdminComplaintTableClick);
  ...
  document.getElementById('notification-table')?.addEventListener('click', handleNotificationTableClick);
