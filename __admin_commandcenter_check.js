...
function exportTabPdf(tabName) {
  ...
  if (!window.jspdf?.jsPDF) {
    showToast('PDF export is not available right now.', 'error');
    return;
  }
  const doc = new window.jspdf.jsPDF();
  let y = 18;
  const write = (text, indent = 14) => {
  ...
  doc.setFontSize(16);
  doc.text(`${payload.title} Export`, 14, y);
  y += 10;
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  payload.metrics.forEach((item) => write(`${item.label}: ${item.value}`));
  payload.tables.forEach((table) => {
    y += 4;
    doc.setFont('helvetica', 'bold');
    write(table.heading);
    doc.setFont('helvetica', 'normal');
    if (table.headers.length) write(table.headers.join(' | '), 18);
    table.rows.forEach((row) => write(row.join(' | '), 18));
  });
  if (!payload.tables.length) {
    payload.textBlocks.forEach((line) => write(line));
  }
  doc.save(`${tabName}_export.pdf`);
}
